...
export function ForgotPassword({ onBack }: ForgotPasswordProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Enviamos el correo al endpoint correspondiente de Express
      await api.post('/auth/forgot-password', { email });

      // Si el servidor responde con éxito, mostramos la vista de confirmación
      setIsSubmitted(true);
    } catch (err: any) {
      // Capturamos el mensaje si el correo no existe o si se cae el servidor
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1a2332] via-[#1e2836] to-[#0f1419] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-[#1e2836]/80 backdrop-blur-xl rounded-2xl p-8 shadow-2xl border border-white/10">
          ...
          <h3 className="text-white text-center mb-6 font-semibold">RECUPERAR CONTRASEÑA</h3>
          ...
          {!isSubmitted ? (
            <>
              ...
              {/* NUEVO: Mensaje de error visual en color rojo */}
              {error && (
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white py-3.5 rounded-lg hover:from-blue-700 hover:to-blue-800 transition-all font-semibold shadow-lg shadow-blue-500/30 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isLoading ? (
                    <>
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-gray-400 text-sm">
                Hemos enviado las instrucciones para restablecer su contraseña a <span className="text-cyan-400">{email}</span>
              </p>
